...
#include "turbulentPotentialBlend.H"
...
defineTypeNameAndDebug(turbulentPotentialBlend, 0);
addToRunTimeSelectionTable(RASModel, turbulentPotentialBlend, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

tmp<volScalarField> turbulentPotentialBlend::Ts() const
{ 
	if(tslimiter_ == "true")
	{
...
tmp<volScalarField> turbulentPotentialBlend::Ls() const
{
...
turbulentPotentialBlend::turbulentPotentialBlend
(
    ...
    )
{
    ...
    //*************************************//
	nut_ = cMu_*k_*tpphi_*k_/epsilon_;	
...
tmp<volSymmTensorField> turbulentPotentialBlend::R() const
{
...
// Not used but necessary for RAS Model
tmp<volSymmTensorField> turbulentPotentialBlend::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> turbulentPotentialBlend::divDevReff() const
{
    return
    (
       fvc::grad(phiReal())
...
bool turbulentPotentialBlend::read()
{
    if (RASModel::read())
    {
...
void turbulentPotentialBlend::correct()
{
    ...
    const dimensionedScalar v0("minVort", vorticity_.dimensions(), SMALL);
    ...
    gamma_ = 1.0/(1.0 + cG_*(nutRe/nu()) + gammaWall);
    lambda_ = 1.0/(1.0 + cGp_*sqrt(nutRe/nu()) + lambdaWall);
    ...
    tmp<fvScalarMatrix> betaEqn
    (
      - fvm::laplacian(beta_)
     ==
      - fvm::Sp(1.0/sqr(L), beta_)
      + 1.0/((epsilon_*T)*sqr(L))
    );
    ...
    volScalarField kbeta("kbeta", min(1.0,k_*beta_));
    ...
    ( 
      ...
      - kbeta*cP1_*bphi*epsHat_

      // Pressure Strain Fast
	  + cP2_*tpProd_*tpphi_

	  // Non-linear pressure strain
	  + kbeta*cD3_*(sqr(bphi) + (tppsi_ & tppsi_) - (2.0/3.0)*IIb)*epsHat_
      //- (1.0 - k_*beta_)*2.0*nu()*(gradTpphiSqrt & gradTpphiSqrt)
      
	  // From K eqn + pressure strain fast
      - fvm::Sp(tpProd_,tpphi_)
	  
	  // Dissipation (Commented as taken into account in cP1eqn)
	  + alpha_*kbeta*bphi*epsHat_
      - (1.0 - kbeta)*epsilon_*tpphi_/(k_ + k0_)
    ...
    (
      ...
      - fvm::Sp(cP1_*kbeta*epsHat_,tppsi_)
	  + kbeta*cD4_*(bphi + (uudk-(2.0/3.0)))*epsHat_*tppsi_
      //- (1.0 - kbeta)*epsilon_*tppsi_/(k_ + k0_)

	  // Fast Pressure Strain
	  - cP2_*tpphi_*vorticity_
      + cP2_*tpProd_*tppsi_ 
      
      - fvm::Sp(cP3_*2.0*alpha_*tpProd_,tppsi_) 
      //- cP3_*(1.0-lambda_)*sqrt(1.0-alpha_+tph0)*vorticity_
 
      // Fix for recirculating flows
    ...
    //volScalarField cMuEqn("cMuEqn", min((cN2_*cMu_*alpha_/alpha_),(tppsi_ & tppsi_)*epsilon_/(phiActual*mag(tpProd_) + epsilonSmall_)));
	...
	Info<< "* * * * * * * * * * * * * * * * * * * * * * * * * * * * * *" << endl; 
    Info<< "Max cMu comb: " << gMax(cMuRe_) << " Min cMu comb: " << gMin(cMuRe_) << " Average cMu comb: " << gAverage(cMuRe_) << endl;
	Info<< "Max cEp1: " << gMax(cEp1calc) << " Min cEp1: " << gMin(cEp1calc) << endl;  
    Info<< "Max kbeta: " << gMax(kbeta) << " Min kbeta: " << gMin(kbeta) << endl; 
